...
class RenderTexture{
public:
	RenderTexture(IRenderCore *irc) {
		m_pIrc = irc;
	}

	void AddTexture(VertexData vertexs[], Texture *bmpTex, int vectexCount) {
		for (int i = 0; i < vectexCount / 3; ++i) {
			int index = i * 3;

			if (vertexs[index].isCull || vertexs[index + 1].isCull || vertexs[index + 2].isCull)
				continue;
			DrawTriangle(vertexs[index], vertexs[index+1], vertexs[index+2]);
		}
	}

	void DrawTriangle(VertexData vd1, VertexData vd2, VertexData vd3) // 画任意实心三角形  
	{
		if (vd1.pos.y == vd2.pos.y)
		{
			if (vd3.pos.y > vd1.pos.y) // 平底
			{
				RenderTexture1(vd3, vd1, vd2);
			}
			else // 平顶  
			{
				RenderTexture2(vd3, vd1, vd2);
			}
		}
		else if (vd1.pos.y == vd3.pos.y)
		{
			...
			{
				RenderTexture1(vd2, vd1, vd3);
			}
			else // 平顶  
			{
				RenderTexture2(vd2, vd1, vd3);
			}
		}
		else if (vd2.pos.y == vd3.pos.y)
		{
			...
			{
				RenderTexture1(vd1, vd2, vd3);
			}
			else // 平顶  
			{
				RenderTexture2(vd1, vd2, vd3);
			}
		}
		else
		{
			...
			// 画平底  
			RenderTexture1(topV, newV, middleV);

			// 画平顶  
			RenderTexture2(bottomV, newV, middleV);
		}
	}
...
private:
	IRenderCore *m_pIrc;
	// 绘制平底三角纹理 （顶 左下 右下）下
	void RenderTexture1(VertexData vd1, VertexData vd2, VertexData vd3) {
	...
	// 绘制平顶三角形（下 左上  右上）
	void RenderTexture2(VertexData vd1, VertexData vd2, VertexData vd3) {
		double x, y, xleft, xright;				// 插值x和y，左右线段x
		double oneoverz_left, oneoverz_right;	// 左右线段1/z
		double oneoverz_top, oneoverz_bottom;	// 上下顶点1/z
		double oneoverz, oneoverz_step;			// 插值1/z以及扫描线步长
